...
OUTPUT_FILE = PROJECT_BASE_PATH / 'data' / 'output' / \
    f'consumer-and-service-report-{datetime.now()}.xlsx'
...
if __name__ == '__main__':
    ...
    # Renames columns to stadardize underline as words separator instead of
    # dot or hyphen
    ...
    date_columns = [
        ...
        'route_created_at',
        'started_at']
    df_standardized_dates = etl_helper.from_unix_to_datetime(
        df=df, date_columns=date_columns)
    ...
    json_columns = [
        ...
        'route_protocols',
        'request_querystring']
    list_columns_converted = etl_helper.from_list_to_json(
        df=df_standardized_dates, json_columns=json_columns)

    # Loads data to db
    etl_helper.load_to_mysql(
        db_conn=MYSQL_CONN,
        df=list_columns_converted,
        table="logs")
    ...
    # Get data from db
    customer_report = etl_helper.query_mysql(
        db_conn=MYSQL_CONN, query_name=CONSUMER_QUERY)
    service_report = etl_helper.query_mysql(
        db_conn=MYSQL_CONN, query_name=SERVICE_QUERY)

    # Write report to file
    etl_helper.write_df_to_xlsx(
        data=[
            customer_report,
            service_report],
